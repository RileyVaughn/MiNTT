...
BenchMark_MiNTT128_simd_int16::BenchMark_MiNTT128_simd_int16(){
    Setup();
}

void BenchMark_MiNTT128_simd_int16::Setup(){
...
void BenchMark_MiNTT128_simd_int16::Hash(uint8_t input[INPUT_SIZE],uint8_t out[OUTPUT_SIZE]){
...
void BenchMark_MiNTT128_simd_int16::ncc(uint8_t input[ndiv8], int16_t intermed[ndiv8][8]){
    ...
    Util16::SIMD_AddSub(intermed[8], intermed[9]);
    Util16::SIMD_AddSub(intermed[10], intermed[11]);
    Util16::SIMD_AddSub(intermed[12], intermed[13]);
    Util16::SIMD_AddSub(intermed[14], intermed[15]);
    

    Util16::SIMD_LShift(intermed[3],4);
    Util16::SIMD_LShift(intermed[7],4);
    Util16::SIMD_LShift(intermed[11],4);
    Util16::SIMD_LShift(intermed[15],4);
    ...
    Util16::SIMD_Q_reduce(intermed[11]);
    Util16::SIMD_Q_reduce(intermed[15]);
    auto mod_end = high_resolution_clock::now();
    modulo_time += duration_cast<nanoseconds>(mod_end - mod_start).count();

    ntt_start = high_resolution_clock::now();
    ...
    Util16::SIMD_AddSub(intermed[8], intermed[10]);
    Util16::SIMD_AddSub(intermed[9], intermed[11]);
    Util16::SIMD_AddSub(intermed[12], intermed[14]);
    Util16::SIMD_AddSub(intermed[13], intermed[15]);

    Util16::SIMD_LShift(intermed[5],2);
    Util16::SIMD_LShift(intermed[6],4);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();

    mod_start = high_resolution_clock::now();
    Util16::SIMD_Q_reduce(intermed[7]);
    mod_end = high_resolution_clock::now();
    modulo_time += duration_cast<nanoseconds>(mod_end - mod_start).count();

    ntt_start = high_resolution_clock::now();
    Util16::SIMD_LShift(intermed[7],6);
    Util16::SIMD_LShift(intermed[13],2);
    Util16::SIMD_LShift(intermed[14],4);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();
    ...
    ntt_start = high_resolution_clock::now();
    Util16::SIMD_LShift(intermed[15],6);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();
    ...
    Util16::SIMD_Q_reduce(intermed[7]);
    Util16::SIMD_Q_reduce(intermed[13]);
    Util16::SIMD_Q_reduce(intermed[14]);
    Util16::SIMD_Q_reduce(intermed[15]);
    mod_end = high_resolution_clock::now();
    modulo_time += duration_cast<nanoseconds>(mod_end - mod_start).count();
    ...
    Util16::SIMD_AddSub(intermed[8], intermed[12]);
    Util16::SIMD_AddSub(intermed[9], intermed[13]);
    Util16::SIMD_AddSub(intermed[10], intermed[14]);
    Util16::SIMD_AddSub(intermed[11], intermed[15]);

    Util16::SIMD_LShift(intermed[9],1);
    Util16::SIMD_LShift(intermed[10],2);
    Util16::SIMD_LShift(intermed[11],3);
    Util16::SIMD_LShift(intermed[12],4);
    Util16::SIMD_LShift(intermed[13],5);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();
    ...
    ntt_start = high_resolution_clock::now();
    Util16::SIMD_LShift(intermed[14],6);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();
    ...
    ntt_start = high_resolution_clock::now();
    Util16::SIMD_LShift(intermed[15],1);


    mod_start = high_resolution_clock::now();
    Util16::SIMD_Q_reduce(intermed[9]);
    Util16::SIMD_Q_reduce(intermed[10]);
    Util16::SIMD_Q_reduce(intermed[11]);
    Util16::SIMD_Q_reduce(intermed[12]);
    Util16::SIMD_Q_reduce(intermed[13]);
    Util16::SIMD_Q_reduce(intermed[14]);
    Util16::SIMD_Q_reduce(intermed[15]);
    mod_end = high_resolution_clock::now();
    ...
    ntt_start = high_resolution_clock::now();
    Util16::SIMD_AddSub(intermed[0], intermed[8]);
    Util16::SIMD_AddSub(intermed[1], intermed[9]);
    Util16::SIMD_AddSub(intermed[2], intermed[10]);
    Util16::SIMD_AddSub(intermed[3], intermed[11]);
    Util16::SIMD_AddSub(intermed[4], intermed[12]);
    Util16::SIMD_AddSub(intermed[5], intermed[13]);
    Util16::SIMD_AddSub(intermed[6], intermed[14]);
    Util16::SIMD_AddSub(intermed[7], intermed[15]);
    ntt_end = high_resolution_clock::now();
    other_ntt_time += duration_cast<nanoseconds>(ntt_end - ntt_start).count();
...
void BenchMark_MiNTT128_simd_int16::ntt_sum(uint8_t input[INPUT_SIZE], int16_t out[d][ndiv8][8]){
...
void BenchMark_MiNTT128_simd_int16::change_base(int16_t val[d][ndiv8][8], uint8_t out[OUTPUT_SIZE]){

    for (size_t i = 0; i < d; i++){
        for (size_t j = 0; j < ndiv8; j++){
            auto mod_start = high_resolution_clock::now();
            Util16::SIMD_Mod257(val[i][j]);
            auto mod_end = high_resolution_clock::now();
            modulo_time += duration_cast<nanoseconds>(mod_end - mod_start).count();
...
void BenchMark_MiNTT128_simd_int16::PrintBenchMark(){
