...
MiNTT64_SIMD_int64::MiNTT64_SIMD_int64(){
    Setup();
}

void MiNTT64_SIMD_int64::Setup()Setup(){
...
void MiNTT64_SIMD_int64::Hash(uint8_t input[INPUT_SIZE],uint8_t out[OUTPUT_SIZE]){
...
void MiNTT64_SIMD_int64::ncc(uint8_t input[ndiv8], int64_t intermed[ndiv8][8]){
...
void MiNTT64_SIMD_int64::ntt_sum(uint8_t input[INPUT_SIZE], int64_t out[d][ndiv8][8]){
...
void MiNTT64_SIMD_int64::change_base(int64_t val[d][ndiv8][8], uint8_t out[OUTPUT_SIZE]){

    for (size_t i = 0; i < d; i++){
        for (size_t j = 0; j < ndiv8; j++){
            Util64::SIMD_Mod257(val[i][j]);
