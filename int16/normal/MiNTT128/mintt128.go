...
func ncc(input [ndiv8]byte) [ndiv8][8]int16 {
	...
	for k := int16(0); k < ndiv8; k++ {
		...
		util.Fake_SIMD_Q_reduce(&intermed[k])
	...
	util.Fake_SIMD_Q_reduce(&intermed[3])
	util.Fake_SIMD_Q_reduce(&intermed[7])
	util.Fake_SIMD_Q_reduce(&intermed[11])
	util.Fake_SIMD_Q_reduce(&intermed[15])
	...
	util.Fake_SIMD_Q_reduce(&intermed[5])
	util.Fake_SIMD_Q_reduce(&intermed[6])
	util.Fake_SIMD_Q_reduce(&intermed[7])
	util.Fake_SIMD_Q_reduce(&intermed[13])
	util.Fake_SIMD_Q_reduce(&intermed[14])
	util.Fake_SIMD_Q_reduce(&intermed[15])
	...
	util.Fake_SIMD_Shift(&intermed[15], 6)
	util.Fake_SIMD_Q_reduce(&intermed[15])
	util.Fake_SIMD_Shift(&intermed[15], 1)

	util.Fake_SIMD_Q_reduce(&intermed[9])
	util.Fake_SIMD_Q_reduce(&intermed[10])
	util.Fake_SIMD_Q_reduce(&intermed[11])
	util.Fake_SIMD_Q_reduce(&intermed[12])
	util.Fake_SIMD_Q_reduce(&intermed[13])
	util.Fake_SIMD_Q_reduce(&intermed[14])
	util.Fake_SIMD_Q_reduce(&intermed[15])
